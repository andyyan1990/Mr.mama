...
/**
 Adds category filter. Only results with given category will be returned.
...
/**
 Adds custom filter by tag (value of tag is exactly matches to given value)
...
/**
 Adds custom filter by tag (value of tag exactly matches to one of given values)
...
/**
 ...
 @param customObjects Custom objects to add
 @param infoBlock Block called for each custom result to fill category and location.
 @param completionBlock Block will be called when search is finished.
 */
-(void) startWithCustomObjects:(NSArray<id> *_Nullable)customObjects infoBlock:(GLMapSearchInfoBlock _Nullable)infoBlock completionBlock:(GLMapSearchCompletionBlock _Nonnull)completionBlock;
